...
#include <stdlib.h>
#include <string.h>
//#include "Debug.h" //add in 20171113 for OVS_LOG_INFO()
...
//void print_ip_into_file(ovs_16aligned_be32 ip)
//{
//    FILE *fp; //20171113 add for file prinFILE *fp; 
//    unsigned char bytes[4];
//    bytes[0] = ip & 0xFF;
//    bytes[1] = (ip >> 8) & 0xFF;
//    bytes[2] = (ip >> 16) & 0xFF;
//    bytes[3] = (ip >> 24) & 0xFF;   
//    fp = fopen("flow_c.txt","a");
...
miniflow_extract(struct dp_packet *packet, struct miniflow *dst)
{
    ...
    FILE *fp; //20171113 add for file print
  ...
  /* Metadata. */
    ...
    if (OVS_LIKELY(dl_type == htons(ETH_TYPE_IP))) {
        ...
        fp = fopen("flow_c.txt","a");
        fprintf(fp,"ip_src   =  %ld \n", nh->ip_src); 
        fprintf(fp,"ip_dst   =  %ld \n", nh->ip_dst); 
        fprintf(fp,"length   =  %d  \n", ntohs(nh->ip_tot_len));
		fprintf(fp,"--------------------------------  \n");
        fclose(fp);
		...
		//OVS_LOG_INFO("src_ip_20171111");
		//OVS_LOG_INFO("dst_ip_20171111");
        //fp = fopen("flow_c.txt","a");
        //fprintf(fp,"tot_len=  %d \n", tot_len);
        //fprintf(fp,"-------------  \n");
        //fclose(fp);
